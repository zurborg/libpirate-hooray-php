...
class Str
{
     ...
     * @return string
     */
    public static function pluralize($text, $amount, $search = '$')
    {
        self::replace(
            $text,
            '/ \( ( [^|]+? ) \) /x',
     ...
     * @return int[]
     */
    public static function timechunks($seconds)
    {
        if ($seconds < 0) {
            $seconds = 0 - $seconds;
        }
     ...
     * @return string
     */
    public static function duration($seconds, $precision = 2, $locale = null)
    {
        if (!$seconds) {
            return '';
        }
